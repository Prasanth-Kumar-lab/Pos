...
class ProductController extends GetxController {
  ...
  RxList<Product> cartItems = <Product>[].obs; // New: for fetched cart items
  RxBool isLoading = false.obs;
  RxString errorMessage = ''.obs;
  RxString searchQuery = ''.obs;
  RxString cartId = '1'.obs; // Store dynamic cart_id
  RxInt cartItemCount = 0.obs; // New: for cart count

  // Dynamic parameters for API calls
  final String businessId;
  final String billerId; // Using user_id as biller_id
  ...
  void onInit() {
    ...
    fetchProducts().then((_) {
      fetchCartItems(); // Sync cart items and quantities
      fetchCartCount();
    });
  ...
  Future<void> fetchCartItems() async {
    try {
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        print('Decoded JSON: $data'); // Debug the full response
        ...
        if (data is Map<String, dynamic> && data['status'] == 'success') {
          ...
          for (var cartItem in cartItems) {
            ...
            if (matchingProduct.productId != null || matchingProduct.itemName != null) {
              ...
              // Fill missing fields from local product if not in cart response
          ...
          fetchCartCount(); // Sync count after fetching items
        } else {
      ...
      print('Exception during fetchCartItems: $e'); // Debug exception
    }
  }
    ...
    try {
      final response = await http.post(
        ...
        body: {
          ...
          'product_id': product.productId ?? product.itemName, // Use productId if available
          'gst_type': 'NO_GST',
        },
      );
      ...
      if (response.statusCode == 200) {
        try {
          ...
          if (responseData['status'] == 'Success' ||
            ...
            // Store cart_item_id in the product
      ...
      fetchCartCount(); // Sync count after operation
    }
  }
      ...
      try {
        final response = await http.post(
          ...
          body: {
            ...
            // Uncomment if API requires cart_item_id
  ...
  Future<void> clearCart() async {
    try {
      final response = await http.post(
        Uri.parse(ApiConstants.ordersEndPoint),
        body: {
          'delete_cart_id': cartId.value, // Use dynamic cart_id
          'business_id': businessId,
        ...
        try {
          ...
          if (responseData['status'] == 'Success' ||
            ...
            for (var product in products) {
              product.quantity = 0;
              product.cartItemId = null; // Clear cart_item_id
            }
            cartId.value = '1'; // Reset cart_id
            products.refresh();
            filteredProducts.refresh();
